...
├── src/                      # Source code
│   ├── __init__.py
│   ├── paths.py              # Project paths (data/, results/)
...
All plots are generated by `src/visualize.py` and saved to `results/`.
...
python -m src.data_collection   # → data/raw/raw_merged.csv
python -m src.pipeline          # → data/processed/*.csv
python -m src.visualize         # → results/*.png
```
...
1. **src/data_collection.py** → Fetches from NOAA GML, NASA GISS, NCEI, OWID; merges to ≥1000 samples → `data/raw/raw_merged.csv`.
2. **src/pipeline.py** → Loads raw data, engineers features, trains models, exports → `data/processed/*.csv`.
3. **src/visualize.py** → Reads processed CSVs, generates plots → `results/*.png`.
